...
def main():
    ...
    add_h(doc, "VibeStart â€” Technical documentation", 0)
    add_p(
        doc,
        "Version 1.0 â€” use cases, architecture, workflow, tech stack, and code structure "
    ...
    add_p(doc, "3) User triggers an analysis -> POST /api/analyze/{key}. Competitor context is used only when key is comparison; all other panels are product-only.")
    ...
    add_p(doc, "5) Creative run -> POST /api/creative/ad or /api/creative/story using section-1 startupText/startupUrl overlays plus optional profile context (no separate creative URL input).")

    # --- Tech flow ---
    add_h(doc, "# Tech flow (request/response + files)", 1)
    add_p(
        doc,
        "Frontend (App + CreativeStudio) -> API helpers -> Vite dev proxy -> Express routes (routes.js + creativeRoutes.js) -> "
        "scraper/openai/pdf services -> JSON response -> rendered cards/markdown + creative image URLs.",
    )
    ...
    methods = [
        "# (bootstrap) â€” Wires CORS, JSON body parser, health route, API router, JSON parse error handler, global JSON error handler, and listens on HOST:PORT with long-lived HTTP timeouts.",
        "# GET /api/health â€” Returns a small JSON payload confirming the API process is running.",
    ]
    for m in methods:
    ...
    for m in [
        "# getDb â€” Returns the singleton better-sqlite3 Database after ensuring the data directory exists and creating core tables if missing.",
        "# resetAllStoredData â€” Deletes all rows in core tables and clears backend/data/raw_html so each run can start clean.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/httpError.js", 2)
    for m in [
        "# HttpError constructor â€” Builds an Error subclass carrying HTTP status and optional detail for consistent API error responses.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/asyncRoute.js", 2)
    for m in [
        "# asyncRoute(handler) â€” Wraps an async Express handler so rejected promises call next(err) instead of crashing the connection.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/scraper.js", 2)
    for m in [
        "# ensureRawDir â€” Creates backend/data/raw_html if it does not exist so HTML files can be written.",
        "# normalizeUrl â€” Trims input, adds https:// when no scheme is present, and rejects empty strings.",
        "# scrapeUrl â€” HTTP GETs a page, saves full HTML to disk, parses title/description/body text with cheerio, and inserts a scraped_pages row.",
        "# scrapeMany â€” Sequentially scrapes a list of URLs, collecting per-URL success objects or error messages without stopping the batch.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/openaiService.js", 2)
    for m in [
        "# getClient â€” Lazily constructs a single OpenAI client or throws HttpError if OPENAI_API_KEY is missing.",
        "# jsonChat â€” Sends a system+user chat with JSON response_format (and optional max_tokens) and parses the assistant message into an object.",
        "# normalizeDeepDives â€” Coerces competitor_deep_dives into an array sized to the competitor list so multi-competitor comparison renders reliably.",
        "# buildCombinedContext â€” Calls the LLM to merge founder text with a scrape summary into one markdown brief plus assumptions and tags.",
        "# comparisonAndBrand â€” Produces exhaustive multi-competitor comparison tables plus one deep-dive card per competitor, along with brand strategy outputs.",
        "# roastStartup â€” Returns structured skeptical VC-style feedback including verdict, fixes, and positioning tweaks.",
        "# jobsFromCompetitors â€” Produces hiring thesis, organizational gaps, detailed job opportunities, and interview scorecard informed by competitors.",
        "# potentialVCsWithMatch â€” Suggests overlooked VC/investor targets with match_score 0â€“100 and outreach hooks.",
        "# strategicGapsAnalysis â€” Identifies product/team/market/fundraising gaps with severity and a 90-day closure plan.",
        "# startupIdeasTailored â€” Generates competitor-whitespace startup ideas with fit scores and validation steps.",
        "# collaborationOpportunities â€” Suggests ethical partnership types, pitches, and boundaries.",
        "# uniqueExtras â€” Outputs moat experiments, pivot seeds, community and pricing hypotheses, and metric ideas.",
    ]:
        add_method_line(doc, m)
    ...
    for m in [
        ...
        "# loadContextBundle - Loads profile summary and competitor summaries (competitors are consumed only by deep comparison).",
        "# POST /api/profile - Upserts a profile, attempts URL scrape, builds combined summary, and still saves with scrapeWarning when URL fetch fails.",
        ...
        "# POST /api/analyze/:key - Dispatches intelligence runs; competitor context is passed only to comparison; snapshot is persisted.",
        "# POST /api/simulate/vc - Runs VC role-play from startup brief and optional pitch notes.",
        "# POST /api/export/pdf - Exports all intelligence sections in panel order with optional latest-snapshot merge.",
        "# GET /api/snapshots/:profileId - Returns recent snapshots with safe JSON parsing per row.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/creativeRoutes.js", 2)
    for m in [
        "# POST /api/creative/ad - Builds creative context from section-1 overlay values and/or saved profile, then generates ad copy (+ optional image).",
        "# POST /api/creative/story - Uses the same context strategy to generate narrative text or image storyboard panels.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## backend/src/creativeService.js", 2)
    for m in [
        "# getLatestStoredSiteBody - Reads latest persisted startup scrape text as a context fallback.",
        "# loadCreativeContextForCreative - Resolves context from current startupText/startupUrl overlay, saved profile fields, and stored scrape fallback.",
        "# generateAdFromProduct - Produces structured ad copy and optionally one generated image.",
        "# generateStoryFromProduct - Produces story markdown or panelized image storyboard outputs.",
    ]:
        add_method_line(doc, m)
    ...
    for m in [
        ...
        "# runAnalysis - POSTs profileId (+ optional extras) to /api/analyze/:key.",
        "# simulateVc - POSTs VC persona and pitch notes to /api/simulate/vc.",
        "# creativeAd / creativeStory - POST creative requests with startup overlay data from section 1.",
        "# exportReportPdf - Calls /api/export/pdf and downloads the generated report.",
    ]:
        add_method_line(doc, m)
    ...
    for m in [
        ...
        "# ComparisonView - Renders deep multi-competitor comparison tables, stage and ahead/behind sections, and per-competitor deep-dive cards.",
        "# RoastView - Renders VC-style roast output with severity and a one-liner upgrade pullquote.",
        "# JobsView - Renders organizational gaps and role plans using product-only context fields.",
    ...
    for m in [
        ...
        "# useEffect (startup) - Validates saved profileId on load and clears local state if backend data was reset.",
        ...
        "# run - Clears old outputs, calls runAnalysis for a given key, and updates panel state.",
        "# CreativeStudio integration - Passes startupText/startupUrl overlays so creative generation works before save.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## frontend/src/main.jsx", 2)
    for m in [
        "# (bootstrap) â€” Mounts the App root component into #root under React StrictMode.",
    ]:
        add_method_line(doc, m)

    add_h(doc, "## frontend/vite.config.js", 2)
    for m in [
        "# defineConfig â€” Enables React plugin and proxies /api to 127.0.0.1:3001 with extended timeouts and proxy error JSON fallback.",
    ]:
        add_method_line(doc, m)
